  ...
  if (participantes.length > 32) {
    ...
    for (let i = 0; i < partsis; i++) {
      ...
      let gol1 =
        ...
        (liga2att[p1 + "POW"] > liga2att[p2 + "POW"] ? 1 : 0); // já que só equipas da segunda liga podem participar nos playoffs, metemos o POW da segunda liga
      let gol2 =
        Math.floor(Math.random() * 4) +
        (liga2att[p2 + "POW"] > liga2att[p1 + "POW"] ? 1 : 0); // e sim, não há vantagem da casa

      if (gol1 == gol2) {
        Math.random() > 0.5 ? gol1++ : gol2++;
      }
      if (gol1 > gol2) {
        participantes.unshift(p2);
        console.log(`${p1} ${gol1} - ${gol2} ${p2}`);
      } else if (gol2 > gol1) {
        participantes.unshift(p1);
        console.log(`${p1} ${gol1} - ${gol2} ${p2}`);
      }
    }
    ...
    while (participantes.length > 1) {
      ...
      let fases = {
        ...
        4: "Semi-finais",
        2: "Final",
      }; // O GRANDE VENCEDOOOOOOOR
      let fasesfasosfaçoslaços =
        fases[participantes.length] || `${fase}ª Eliminatória`;
      console.log(
...
function tabela() {
  ...
  const ordenado = Object.entries(pontos).sort(
    (a, b) => b[1] - a[1] || saldogols[b[0]] - saldogols[a[0]],
  );
...
function tabela2() {
  ...
  const ordenado2 = Object.entries(liga2Pontos).sort(
    (a, b) => b[1] - a[1] || liga2saldogols[b[0]] - liga2saldogols[a[0]],
  );
...
function rodarJornada(lista, ptsObj, sgObj, ataquePOW, defesaPOW) {
  ...
  while (baralhado.length >= 2) {
    ...
    let gol1 =
      Math.floor(Math.random() * 4) +
      (ataquePOW[p1 + "POW"] > ataquePOW[p2 + "POW"] ? 2 : 0) -
      (defesaPOW[p2 + "POW"] > defesaPOW[p1 + "POW"] ? 1 : 0); // VANTAGANAAAAA
    let gol2 =
      Math.floor(Math.random() * 4) +
      (ataquePOW[p2 + "POW"] > ataquePOW[p1 + "POW"] ? 1 : 0);
...
function gerirSubidasEDescidas(ord1, ord2) {
  ...
  rebaixados.forEach((equipa) => {
    ...
    delete pontos[nome];
    delete saldogols[nome];
    delete att[nome + "POW"];
    delete def[nome + "POW"];

    liga2Pontos[nome] = 0;
    liga2saldogols[nome] = 0;
    liga2att[nome + "POW"] = novoatt;
    liga2def[nome + "POW"] = novodef;
  ...
  promovidos.forEach((equipa) => {
    ...
    delete liga2Pontos[nome];
    delete liga2saldogols[nome];
    delete liga2att[nome + "POW"];
    delete liga2def[nome + "POW"];

    pontos[nome] = 0;
    saldogols[nome] = 0;
    att[nome + "POW"] = novoatt;
    def[nome + "POW"] = novodef;
...
function mercado() {
  liga1.forEach((e) => {
    ...
    if (Math.random() > 0.9) {
      ...
      if (rondonia > 4.99) {
        alteracaoatt = 3;
      } else if (rondonia > 4.7) {
        alteracaoatt = 2;
      } else if (rondonia > 4.5) {
        alteracaoatt = 1;
      } else if (rondonia > 4.0) {
        alteracaoatt = -5;
      }
      ...
      console.log(
        `Mercado: ${e} ${alteracaoatt + alteracaodef > 0 ? "reforçou-se" : "enfraqueceu-se"} (Novo ataque: ${att[e + "POW"]} defesa: ${def[e + "POW"]})`,
      );
    }
  });
}
function mercado2() {
  liga2.forEach((e) => {
    ...
    if (Math.random() > 0.9) {
      ...
      if (rondonia2 > 4.99) {
        alteracaoatt = 3;
      } else if (rondonia2 > 4.7) {
        alteracaoatt = 2;
      } else if (rondonia2 > 4.5) {
        alteracaoatt = 1;
      } else if (rondonia2 > 4.0) {
        alteracaoatt = -5;
      }
      ...
      console.log(
        `Mercado: ${e} ${alteracaoatt + alteracaodef > 0 ? "reforçou-se" : "enfraqueceu-se"} (Novo ataque: ${liga2att[e + "POW"]} defesa: ${liga2def[e + "POW"]})`,
      );
    }
  });
}
function impossivel() {
  let champion = Object.entries(pontos).sort(
    (a, b) => b[1] - a[1] || saldogols[b[0]] - saldogols[a[0]],
  );
  ...
  if (att[champ + "POW"] > 150) {
    console.log(
      "IMPOSSÍVEL! O " +
        champion[0][0].toUpperCase() +
        " GANHOU A LIGA DOS CAMPEÕES!",
    );
  }
}
function guardalistadoschamps() {
  let champ1 = Object.entries(pontos).sort(
    (a, b) => b[1] - a[1] || saldogols[b[0]] - saldogols[a[0]],
  )[0];
  let champ2 = Object.entries(liga2Pontos).sort(
    (a, b) => b[1] - a[1] || liga2saldogols[b[0]] - liga2saldogols[a[0]],
  )[0];
...
function iniciarTemporada() {
  ...
  rl.question(
    "\n[1] Próxima Época | [2] Ver a lista dos campeões | [3] Sair: ",
    (opcao) => {
      /*if (opcao === '4') {
            let toje = Date.now()
